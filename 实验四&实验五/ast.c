  ...
  if (T)
    ...
    case DEC_LIST:      
        ...
        while (T0) {
            ...
            {
                if(T0->ptr[0]->ptr[0]->kind == ARRAY_DF) display(T0->ptr[0]->ptr[0],indent); //var_dec assignop exp的情况
	...
	case INT:	     
        ...
        break;
	case FLOAT:	        
        printf("%*cFLAOT 浮点型常量：%f\n",indent,' ',T->type_float);
        break;
    case CHAR:
        printf("%*cCHAR 单字符常量: %c\n", indent, ' ', T->type_char);
        break;
    case BOOL:
        printf("%*cBOOL 布尔常量: %s\n", indent, ' ', T->type_bool);
        break;
    case STRING:
        printf("%*cSTRING 字符串常量: %s\n", indent, ' ', T->type_string);//STRING
        break;
	case ASSIGNOP:
        T0 = T;
