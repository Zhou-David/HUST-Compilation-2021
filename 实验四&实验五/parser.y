...
%token <type_string> STRING          /*指定STRING的语义值是type_id，有词法分析得到的标识符字符串*/
%token <type_bool> BOOL
%token <type_int> INT              /*指定INT的语义值是type_int，有词法分析得到的数值*/
%token <type_id> ID ERRORID RELOP TYPE    /*指定ID,RELOP,TYPE,STRING 的语义值是type_id，有词法分析得到的标识符字符串*/
...
%token ARRAY_DEC ARRAY_DF ARRAY_CALL //加入Array
...
//规则定义部分
%%
program: ExtDefList    {/*displayRoot();display($1,0);*/semantic_Analysis0($1);}     //显示语法树,语义分析
         ; 
ExtDefList: {$$=NULL;}
...
Dec:     VarDec  {$$=$1;}
       ...
       ;
Exp_less: Exp ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
      ...
      | ID LB Exp_less RB    {$$=mknode(1,ARRAY_CALL,yylineno,$3);strcpy($$->type_id,$1);} //数组
